...
func TestNewActivity(t *testing.T) {
	...
	at := Attendee{Name: "John Doe", Email: "john@email.com", ActivityID: 1}
	...
	a, err = NewActivity("Atividade 1", "Descrição atividade 1", 100.0, c.ID, []TimeBlock{tb}, []Attendee{at})
	assert.NotNil(t, tb)
	assert.NoError(t, err)
	assert.NotNil(t, a)
	...
	a, err = NewActivity("", "Descrição atividade 1", 100.0, c.ID, []TimeBlock{tb}, []Attendee{at})
	assert.ErrorIs(t, err, ErrActivityNameIsRequired)
	assert.Nil(t, a)

	a, err = NewActivity("Atividade 1", "", 100.0, c.ID, []TimeBlock{tb}, []Attendee{at})
	assert.ErrorIs(t, err, ErrActivityDescriptionIsRequired)
	assert.Nil(t, a)

	a, err = NewActivity("Atividade 1", "Descrição atividade 1", 0, c.ID, []TimeBlock{tb}, []Attendee{at})
	assert.ErrorIs(t, err, ErrActivityPriceIsRequired)
	assert.Nil(t, a)

	a, err = NewActivity("Atividade 1", "Descrição atividade 1", -10, c.ID, []TimeBlock{tb}, []Attendee{at})
	assert.ErrorIs(t, err, ErrActivityPriceIsInvalid)
	assert.Nil(t, a)

	a, err = NewActivity("Atividade 1", "Descrição atividade 1", 100.0, 0, []TimeBlock{tb}, []Attendee{at})
